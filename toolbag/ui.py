...
'''
    ...
    def PopulateForm(self):
        ...
        p.setColor(QtWidgets.QPalette.Base, self.ui_obj.options['background_color'])
        p.setColor(QtWidgets.QPalette.WindowText, self.ui_obj.options['font_color'])
...
class UI(PluginForm):
        ...
        class rightclicka_vtrace(QtCore.QObject):
            def eventFilter(self, obj, event):
                if event.type() == QtCore.QEvent.ContextMenu:
                    ...
                    obj.activated.connect(edit_action, QtCore.SIGNAL("triggered()"), self.ui.editVtraceScript)
                    obj.activated.connect(run_action, QtCore.SIGNAL("triggered()"), self.ui.runVtraceScript)
                    obj.activated.connect(process_action, QtCore.SIGNAL("triggered()"), self.ui.processVTraceScript)
                    obj.activated.connect(prep_action, QtCore.SIGNAL("triggered()"), self.ui.prepVtraceScript)
    ...
    def saveToRetVals(self):
        ...
        fname, ok = QtGui.QInputDialog().getText(None, "Save to variable", "Enter variable name:", QtWidgets.QLineEdit.Normal, "")
    ...
    def OnClose(self, form):
        # scra debugging
        f = open('C:\dbg\out.txt', 'a')
        f.write('='*30+'\n')
        print "Entering OnClose"
        if self.options['dev_mode']:
        ...
        try:
            self.myhost.end()
            f.write('[*] Server object deleted \n')
            print "[*] Server object deleted"
        except Exception as detail:
            f.write('[*] Server object ERROR \n')
            f.write(str(detail))
            pass

        try:
            del self.myhost
            f.write('[*] myHost deleted \n')
        except:
            pass
        ...
        del(self.fs)
        del(self.reftree)
        f.write('fs and reftree deleted \n')

        # delete any Query objects we created
        # prevents IDA from crashing due to a dangling reference
        f.write('deleting windows \n')
        f.write(str(self.new_windows))
        for c in self.new_windows:
            try:
                f.write(type(c)+'\n') 
                del(c)
            except:
                f.write('passed '+type(c)+'\n')  
                pass
        
        # remove the UI hooks
        f.write('unhooking stuff \n')
        self.ui_hook.unhook()
        #del self.ui_hook
        f.write('XXXXXXXXX donso  \n')
    ...
    def saveQueueData(self):
        ...
        fname, ok = QtGui.QInputDialog().getText(None, "Store Object", "Enter filename:", QtWidgets.QLineEdit.Normal, objname)
